...
# 4a. 로컬 모드 (기본값 — 내 MCP 클라이언트를 위한 stdio 서버)
uv run awx-mcp

# 4b. 팀을 위한 중앙 서버 (관리자): 서버 하나를 공유, 토큰은 사용자별
uv run awx-mcp --serve --host 0.0.0.0 --port 8443
#    엔드포인트: http://HOST:8443/mcp  (--sse를 추가하면 http://HOST:8443/sse)

# 4c. 중앙 서버에 접속 (사용자): 로컬 서버 없이 프록시로 연결
...
uv run awx-mcp --serve --host 0.0.0.0 --port 8443        # sse 트랜스포트를 쓰려면 --sse 추가
```
